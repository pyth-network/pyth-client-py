...
from pythclient.calendar_full_intervals import (
...
EQUITY_HOLIDAY_NEXT_DAY_EARLY_CLOSE_OPEN_THU_2023_11_23_9_30 = datetime.datetime(
    2023, 11, 23, 9, 30, 0, tzinfo=NY_TZ
)
EQUITY_HOLIDAY_NEXT_DAY_EARLY_CLOSE_CLOSE_THU_2023_11_23_13 = datetime.datetime(
    2023, 11, 23, 13, 0, 0, tzinfo=NY_TZ
)
EQUITY_EARLY_CLOSE_OPEN_FRI_2023_11_24_11 = datetime.datetime(
    2023, 11, 24, 11, 0, 0, tzinfo=NY_TZ
)
EQUITY_EARLY_CLOSE_CLOSE_FRI_2023_11_24_14 = datetime.datetime(
    2023, 11, 24, 14, 0, 0, tzinfo=NY_TZ
)
...
FX_METAL_HOLIDAY_SUN_2023_12_24_17 = datetime.datetime(
    2023, 12, 24, 17, 0, 0, tzinfo=NY_TZ
)
FX_METAL_HOLIDAY_SUN_2023_12_31_17 = datetime.datetime(
    2023, 12, 31, 17, 0, 0, tzinfo=NY_TZ
)

METAL_EARLY_HOLIDAY_MON_2024_1_15_13 = datetime.datetime(
    2024, 1, 15, 13, 0, 0, tzinfo=NY_TZ
)
METAL_EARLY_HOLIDAY_MON_2024_1_15_17 = datetime.datetime(
    2024, 1, 15, 17, 0, 0, tzinfo=NY_TZ
)
METAL_EARLY_HOLIDAY_MON_2024_1_15_18 = datetime.datetime(
    2024, 1, 15, 18, 0, 0, tzinfo=NY_TZ
)
...
RATES_HOLIDAY_NEXT_DAY_EARLY_CLOSE_OPEN_THU_2023_11_23_8 = datetime.datetime(
    2023, 11, 23, 8, 0, 0, tzinfo=NY_TZ
)
RATES_HOLIDAY_NEXT_DAY_EARLY_CLOSE_CLOSE_THU_2023_11_23_13 = datetime.datetime(
    2023, 11, 23, 13, 0, 0, tzinfo=NY_TZ
)
RATES_EARLY_CLOSE_OPEN_FRI_2023_11_24_11 = datetime.datetime(
    2023, 11, 24, 11, 0, 0, tzinfo=NY_TZ
)
RATES_EARLY_CLOSE_CLOSE_FRI_2023_11_24_14 = datetime.datetime(
    2023, 11, 24, 14, 0, 0, tzinfo=NY_TZ
)
...
def test_get_next_market_open():
    ...
    # equity weekend
    assert get_next_market_open(
        "equity", EQUITY_CLOSE_SAT_2023_6_10_17
    ) == format_datetime_to_unix_timestamp(
    ...
    assert get_next_market_open(
        ...
        datetime.datetime(2023, 11, 24, 9, 30, 0, tzinfo=NY_TZ)
    )
    ...
    assert get_next_market_open(
        "fx", FX_METAL_OPEN_WED_2023_6_21_21
    ) == format_datetime_to_unix_timestamp(
    ...
    assert get_next_market_open(
        "fx", FX_METAL_OPEN_WED_2023_6_21_23
    ) == format_datetime_to_unix_timestamp(
        datetime.datetime(2023, 6, 25, 17, 0, 0, tzinfo=NY_TZ)
    )
    assert get_next_market_open(
        "metal", FX_METAL_OPEN_WED_2023_6_21_23
    ) == format_datetime_to_unix_timestamp(
        datetime.datetime(2023, 6, 25, 17, 0, 0, tzinfo=NY_TZ)
    )
    ...
    assert get_next_market_open(
        "fx", FX_METAL_CLOSE_SUN_2023_6_18_16
    ) == format_datetime_to_unix_timestamp(
        datetime.datetime(2023, 6, 18, 17, 0, 0, tzinfo=NY_TZ)
    )
    assert get_next_market_open(
        "metal", FX_METAL_CLOSE_SUN_2023_6_18_16
    ) == format_datetime_to_unix_timestamp(
        datetime.datetime(2023, 6, 18, 17, 0, 0, tzinfo=NY_TZ)
    )
    ...
    assert get_next_market_open(
        "fx", FX_METAL_HOLIDAY_SUN_2023_12_24_17
    ) == format_datetime_to_unix_timestamp(
        datetime.datetime(2023, 12, 25, 17, 0, 0, tzinfo=NY_TZ)
    )
    assert get_next_market_open(
        "metal", FX_METAL_HOLIDAY_SUN_2023_12_24_17
    ) == format_datetime_to_unix_timestamp(
        datetime.datetime(2023, 12, 25, 17, 0, 0, tzinfo=NY_TZ)
    )
    ...
    assert get_next_market_open(
        "fx", datetime.datetime(2023, 12, 25, 8, 15, 0, tzinfo=NY_TZ)
    ) == format_datetime_to_unix_timestamp(
        datetime.datetime(2023, 12, 25, 17, 0, 0, tzinfo=NY_TZ)
    )
    ...
    assert get_next_market_open(
        ...
        datetime.datetime(2023, 1, 1, 17, 0, 0, tzinfo=NY_TZ)
    )
    assert get_next_market_open(
        "metal", FX_METAL_HOLIDAY_SUN_2023_1_1
    ) == format_datetime_to_unix_timestamp(
        datetime.datetime(2023, 1, 1, 17, 0, 0, tzinfo=NY_TZ)
    )
    ...
    # metal early holiday
    assert get_next_market_open(
        "metal", METAL_EARLY_HOLIDAY_MON_2024_1_15_13
    ) == format_datetime_to_unix_timestamp(
        datetime.datetime(2024, 1, 15, 18, 0, 0, tzinfo=NY_TZ)
    )
    assert get_next_market_open(
    ...
    assert get_next_market_open(
        ...
        datetime.datetime(2024, 1, 21, 17, 0, 0, tzinfo=NY_TZ)
    )

    # rates within market hours
    assert get_next_market_open(
        "rates", RATES_OPEN_WED_2023_6_21_12
    ) == format_datetime_to_unix_timestamp(
        datetime.datetime(2023, 6, 22, 8, 0, 0, tzinfo=NY_TZ)
    )

    # rates out of market hours
    assert get_next_market_open(
        "rates", RATES_CLOSE_WED_2023_6_21_17
    ) == format_datetime_to_unix_timestamp(
        datetime.datetime(2023, 6, 22, 8, 0, 0, tzinfo=NY_TZ)
    )
    ...
    assert get_next_market_open(
        ...
        datetime.datetime(2023, 6, 12, 8, 0, 0, tzinfo=NY_TZ)
    )

    # rates holiday
    assert get_next_market_open(
        "rates", RATES_HOLIDAY_MON_2023_6_19
    ) == format_datetime_to_unix_timestamp(
        datetime.datetime(2023, 6, 20, 8, 0, 0, tzinfo=NY_TZ)
    )

    # rates holiday next day early close holiday
    assert get_next_market_open(
        "rates", RATES_HOLIDAY_NEXT_DAY_EARLY_CLOSE_OPEN_THU_2023_11_23_8
    ) == format_datetime_to_unix_timestamp(
        datetime.datetime(2023, 11, 24, 8, 0, 0, tzinfo=NY_TZ)
    )

    # rates early close holiday
    assert get_next_market_open(
        "rates", RATES_EARLY_CLOSE_OPEN_FRI_2023_11_24_11
    ) == format_datetime_to_unix_timestamp(
        datetime.datetime(2023, 11, 27, 8, 0, 0, tzinfo=NY_TZ)
    )
    assert get_next_market_open(
...
def test_get_next_market_close():
    ...
    # equity holiday next day early close holiday
    assert get_next_market_close(
        "equity", EQUITY_HOLIDAY_NEXT_DAY_EARLY_CLOSE_CLOSE_THU_2023_11_23_13
    ) == format_datetime_to_unix_timestamp(
    ...
    assert get_next_market_close(
        "fx", datetime.datetime(2023, 11, 10, 7, 0, 0, tzinfo=NY_TZ)
    ) == format_datetime_to_unix_timestamp(
        datetime.datetime(2023, 11, 10, 17, 0, 0, tzinfo=NY_TZ)
    )
    ...
    assert get_next_market_close(
        ...
        datetime.datetime(2024, 1, 15, 14, 30, 0, tzinfo=NY_TZ)
    )
    assert get_next_market_close(
        "metal", METAL_EARLY_HOLIDAY_MON_2024_1_15_17
    ) == format_datetime_to_unix_timestamp(
        datetime.datetime(2024, 1, 19, 17, 0, 0, tzinfo=NY_TZ)
    )
    assert get_next_market_close(
        "metal", METAL_EARLY_HOLIDAY_MON_2024_1_15_18
    ) == format_datetime_to_unix_timestamp(
    ...
    assert get_next_market_close(
        ...
        datetime.datetime(2023, 6, 21, 17, 0, 0, tzinfo=NY_TZ)
    )
    ...
    assert get_next_market_close(
        ...
        datetime.datetime(2023, 6, 22, 17, 0, 0, tzinfo=NY_TZ)
    )
    ...
    assert get_next_market_close(
        ...
        datetime.datetime(2023, 6, 12, 17, 0, 0, tzinfo=NY_TZ)
    )
    ...
    assert get_next_market_close(
        ...
        datetime.datetime(2023, 6, 20, 17, 0, 0, tzinfo=NY_TZ)
    )

    # rates holiday next day early close holiday
    assert get_next_market_close(
        "rates", RATES_HOLIDAY_NEXT_DAY_EARLY_CLOSE_CLOSE_THU_2023_11_23_13
    ) == format_datetime_to_unix_timestamp(
        datetime.datetime(2023, 11, 24, 13, 0, 0, tzinfo=NY_TZ)
    )
    ...
    assert get_next_market_close(
        ...
        datetime.datetime(2023, 11, 27, 17, 0, 0, tzinfo=NY_TZ)
    )
